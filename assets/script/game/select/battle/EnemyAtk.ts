...
import { TileAct, TileGroup } from "../../../define/TileDefine";
 ...
 * 敵人攻擊
 */
export class EnemyAtk extends WorkChain {
    //
    ...
    protected async business(param: SelectParam): Promise<boolean> {
        ...
        // 四向攻擊
        //param.atkers.forEach(atker => this._cmd.add(atker, new TileAtk()));
        param.enemyAtks.forEach(id => {
            let atker = param.model.getTileById(id);
            this._cmd.add(atker, new TileAtk());
        });
        ...
        param.enemyAtks.forEach(id => {
            let atker = param.model.getTileById(id);
            Object.values(FOUR_DIR).forEach(dir => this.attack(atker, dir));
        });
    ...
    private attack(atker: TileModel, offset: IVec2Like): void {
        ...
        if (defer && defer.group == TileGroup.Player) {
